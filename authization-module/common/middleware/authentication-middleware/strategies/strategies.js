...
const strategyCallback= async (idpId, idpName, username, password,protocolName,done) => {
...
const azureStratBuilder= () => new AzureAdOAuth2Strategy({
    ...
    (accessToken, refreshToken, params, profile, done)=> strategyCallback(params.id_token, 'azureAD', jwt.decode(params.id_token).email, 'null','AzureAD',done));
...
const googleStratBuilder = () =>new GoogleStrategy({clientID: google.google_client_id, clientSecret: google.google_client_secret, callbackURL: google.callbackUrl,},
    (accessToken, refreshToken, profile, done) =>strategyCallback(profile.id, 'google', profile.displayName, 'null','Google',done));
...
const samlStratBuilder = () => new SamlStrategy({callbackUrl:saml.callbackUrl, entryPoint:saml.entryPoint, issuer:saml.issuer, cert: saml.certificate, signatureAlgorithm:'sha256'},
    (profile, done)=>strategyCallback(profile.nameID, 'saml', profile[usernameLink], 'null','Saml',done));
