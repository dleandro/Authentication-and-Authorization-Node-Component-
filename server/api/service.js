...
module.exports = function(data) {
    return {
        ...
        loginUser: async (username, password) => executeRequest( () => data.user.getUser([username, password]) ),
        
        // Creates a new entry on the database with given user parameters
        register: async (username, password) => executeRequest( () => data.user.insertUser([username, password]) ),

        // Requests the dal to delete a specific user resource
        deleteUser: async (userId) => executeRequest( () => data.user.deleteUser([userId] )),
        
        // Get user with specific id
        getUserById: async (id) => executeRequest( () => data.user.getUserById([id]) ),
        
        getUserByEmail: async (email) => ( () => data.user.getUserByEmail([email]) ),
